\documentclass[a4paper, 11pt, toc=listof, toc=bib]{scrbook}

% Vorlage fuer Abschlussarbeiten
% Version 2023-08-19
% Getestet mit TeX Live; Kodierung: UTF8

% Hier Daten eintragen:
\newcommand{\name}{Max Mustermann}
\newcommand{\abschlussarbeit}{Bachelorarbeit}
\newcommand{\hochschule}{Fachhochschule Südwestfalen}
\newcommand{\datum}{\today}
\newcommand{\titeldeutsch}{Titel der Abschlussarbeit}
\newcommand{\titelenglisch}{Englischer Titel der Abschlussarbeit}
\newcommand{\erstpruefer}{Prof. Dr. ???}
\newcommand{\zweitpruefer}{Prof. Dr. ???}
% Ende der Daten

\usepackage[inner=25mm, outer=25mm, top=25mm, lines=50]{geometry}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage[ngerman]{babel}
\usepackage[style=alphabetic, backend=biber, bibencoding=utf8]{biblatex}
\addbibresource{literatur.bib}\usepackage{csquotes}
\usepackage{scrlayer-scrpage}\lohead{\rightmark}\rehead{\leftmark}\ohead{\pagemark}
\usepackage{booktabs}
\usepackage{microtype}
\usepackage{graphicx}
\usepackage{scrhack}
\clubpenalty=10000
\widowpenalty=10000
\displaywidowpenalty=10000
\setlength{\parindent}{1.5em}

\usepackage{listings}
\renewcommand{\lstlistingname}{Listing}
\renewcommand{\lstlistlistingname}{Listingverzeichnis}
\lstset{basicstyle=\small\ttfamily, breaklines=true, numbers=left, keepspaces=true, columns=fixed}

% Konfiguration fuer Markdown:
\usepackage[
    fencedCode,
    pipeTables,
    tableCaptions,
    citations,
    % hybrid % LaTeX innerhalb von Markdown erlauben
    ]{markdown}
\usepackage{float}\floatplacement{figure}{htbp} % Platzierung von Abbildungen und Tabellen
% Abstand vor und nach Aufzaehlungen:
\def\markdownRendererUlBeginTight{\vspace{6pt}\begin{compactitem}}
\def\markdownRendererUlEndTight{\end{compactitem}\vspace{6pt}}
\def\markdownRendererOlBeginTight{\vspace{6pt}\begin{compactitem}}
\def\markdownRendererOlEndTight{\end{compactitem}\vspace{6pt}}
% Ende der Markdown-Konfiguration

\begin{document}
\frontmatter
\include{frontmatter}
\ofoot*{}\ifoot*{}\cfoot*{}
\tableofcontents

\mainmatter
\markdownInput{mainmatter.md}

\backmatter
\printbibliography
\listoffigures
\listoftables
\include{erklaerung}
\end{document}
